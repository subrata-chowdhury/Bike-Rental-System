        ...
        <ul className="nav nav-pills flex-column py-4 px-3 bg-white h-100 border-r border" style={{ minHeight: '100vh' }}>
            {tabs.map((tab, index) => (
                <Link to={tab.href} className="nav-item mb-2" title={tab.href.charAt(0).toUpperCase() + tab.href.slice(1)} key={index}>
                    <div className={`nav-link cursor-pointer bg-glass bg-light-white p-2 d-flex justify-content-center align-items-center ${currentTab.includes(tab.href) ? 'active text-white bg-dark' : ' text-dark'}`}>{tab.icon}</div>
                </Link>
            ))}
